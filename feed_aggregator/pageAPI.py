...
def get_article_data(pk):
    try:
        ...
        article[f'email__link'] = 'mailto:?subject=' + urllib.parse.quote(
            SHARE_EMAIL_SUBJECT) + '&body=' + urllib.parse.quote(SHARE_EMAIL_BODY)
        article[f'teams__link'] = 'https://teams.microsoft.com/l/chat/0/0?users=sven.van.almsick@morganstanley.com&message=' + urllib.parse.quote(
            SHARE_TEAMS_LINK)
        article['error'] = False
    except:
        article = {'error': True}
